...
function animate(){
    ...
    if (changeOccurred) {
        ...
        // tf.loadLayersModel('/model.h5').then(model => {
        //     let input = tf.tensor(grid).reshape([1, 28, 28, 1]);
        //     let predictions = model.predict(input).dataSync();
        //     console.log(predictions);
        //     // Display the predictions
        //     for (let i = 0; i < predictions.length; i++) {
        //     console.log(`Digit ${i}: ${predictions[i] * 100}%`);
        //     }
        // }).catch(err => {
